...
// /api/v1/invoices + the status endpoint).
//
...
// get/list, update (lines rebuilt, DRAFT-only), delete (soft, DRAFT-only), the
// full status lifecycle, the derived display_status, and multi-tenant isolation.
// =============================================================================
...
// id, registering hard-delete cleanup (cascades to invoice_items).
...
func TestHandleCreateInvoice(t *testing.T) {
	...
	t.Run("two lines: totals + VAT correct and persisted", func(t *testing.T) {
		...
		t.Cleanup(func() { _, _ = ts.pool.Exec(context.Background(), "DELETE FROM invoices WHERE id = $1", got.ID) })
	...
	t.Run("missing contact_id → 400 (binding)", func(t *testing.T) {
		rec := postInvoice(t, ts, bearer(t, ts, devUserID, devOrgID), invoices.CreateInvoiceRequest{DatedOn: today()})
	...
	t.Run("malformed contact_id → 400 (binding uuid)", func(t *testing.T) {
		rec := postInvoice(t, ts, bearer(t, ts, devUserID, devOrgID), invoices.CreateInvoiceRequest{ContactID: "not-a-uuid", DatedOn: today()})
		if rec.Code != http.StatusBadRequest {
			t.Errorf("expected 400, got %d — body: %s", rec.Code, rec.Body.String())
		}
	})
	...
	t.Run("bad dated_on → 422", func(t *testing.T) {
		contactID := createContactAs(t, ts, devUserID, devOrgID)
		rec := postInvoice(t, ts, bearer(t, ts, devUserID, devOrgID), invoices.CreateInvoiceRequest{ContactID: contactID, DatedOn: "31/12/2026"})
		if rec.Code != http.StatusUnprocessableEntity {
	...
	t.Run("requires auth → 401", func(t *testing.T) {
		rec := postInvoice(t, ts, "", invoices.CreateInvoiceRequest{ContactID: uuid.NewString(), DatedOn: today()})
...
// 422, independent of the HTTP boundary.
func TestInvoiceService_BadContact_Direct(t *testing.T) {
	...
	_, err := ts.invoiceService.CreateInvoice(
		...
		invoices.CreateInvoiceRequest{ContactID: uuid.NewString(), DatedOn: today()},
	)
...
func TestInvoiceDisplayStatus(t *testing.T) {
	...
	t.Run("SENT with no lines → Zero Value", func(t *testing.T) {
		contactID := createContactAs(t, ts, devUserID, devOrgID)
		rec := postInvoice(t, ts, auth, invoices.CreateInvoiceRequest{ContactID: contactID, DatedOn: today()})
		if rec.Code != http.StatusCreated {
...
func TestInvoices_TenantIsolation(t *testing.T) {
	...
	if rec := putInvoice(t, ts, invoiceA, authB, invoices.UpdateInvoiceRequest{ContactID: contactB, DatedOn: today()}); rec.Code != http.StatusNotFound {
