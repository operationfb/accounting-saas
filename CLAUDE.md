...
├── server.go            # THIN SHELL (post-migration): Gin engine + global CORS + /health + the static-SPA fallback + Router() (the per-domain RegisterRoutes seam). Holds NO services and NO domain routes — every API route is registered by its internal/<domain> Handler from main. NewServer(corsOrigins).
│   # NOTE: auth + user profile were extracted to internal/userauth (2026-06-21):
│   #   internal/userauth/ — AuthHandler (login + forgot/reset password; PUBLIC /api/v1/auth/* via RegisterRoutes) + the profile Service/Handler (GET/PUT /api/v1/profile) + the shared user/org DTOs (UserResponse, OrganisationResponse, NewUserResponse) + the reset-email content (email.go). EmailSender is a consumer-interface SEAM: the concrete senders in **internal/email** (email.Sender interface + logSender/smtpSender, constructors email.NewLogSender / email.NewSMTPSender(email.Config{…}); extracted 2026-06-22) satisfy it, injected by main.
...
│   #   internal/expenses/ — expenses.Service (was ExpenseService) + Handler (self-registers /api/v1/expenses* + /expense-categories + /vat-rates) + dto.go (the request/response DTOs) + expense_status.go (the approval state machine) + events.go/events_pubsub.go (EventPublisher + ExpenseApprovedEvent + the Pub/Sub impl, NewPubSubPublisher; main calls service.SetPublisher after construction). db/expenses is aliased `expensesdb` inside; `expenses` = the domain pkg. CATEGORIES: an expense's `category_id` references the **shared Chart of Accounts** (`categories`, `internal/categories`) — NOT a per-module table (the old `expense_categories` was dropped in the 2026-06-30 unification). The service injects `categoriesdb.Querier`: `GET /expense-categories` returns the SPENDING subset via `ListSpendingCategories` (shared with bills' picker; COST_OF_SALES/ADMIN_EXPENSE/CAPITAL_ASSET), and create/update enforce it with `validateCategory` (mirrors `internal/bills`).
...
│   # NOTE: ATTACHMENTS was extracted to internal/attachments (2026-06-22, Stage 2 of the finale):
│   #   internal/attachments/ — attachments.Service (was AttachmentService) + Handler (self-registers /api/v1/expenses/:id/attachments* + /capture under its OWN auth group; same :id wildcard as internal/expenses' CRUD — Gin merges the two groups) + CaptureFromReceipt ("Smart Upload"). Imports internal/expenses (AttachmentResponse / ExpenseDetailResponse / BuildExpenseDetail / PgDateFromTime), db/expenses as `expensesdb`, internal/storage, internal/ocr.
│   #   Exported for cross-package use: OcrEnqueuer (the consumer seam — main builds it as a true-nil interface to dodge the typed-nil trap), PlaceholderDescription, PlaceholderCategoryNominal, MaxUploadRequestBytes. The withTx tx-helper + parseResourceUUID stayed in-package (unexported).
│   #   email_inbox + integrations now hold *attachments.Service (CaptureFromReceipt / FindDuplicateReceipt = email's receipt-capturer seam; PrimaryAttachmentForPush = integrations' AttachmentFetcher seam).
...
│   #   internal/storage/  — Storage interface + gcsStorage (storage.go + gcs.go; constructor storage.NewGCS; GCS SDK aliased `gcs`). Consumed by internal/attachments.
│   # internal/htmlrender/ — Renderer interface + gotenbergRenderer (HTML email body → PDF via Gotenberg; constructor htmlrender.NewGotenberg). Consumed by internal/emailinbox for the no-attachment fallback.
│   # NOTE: EMAIL-INBOX was extracted to internal/emailinbox (2026-06-22, Stage 3 — the finale):
│   #   internal/emailinbox/ — emailinbox.Service (was EmailInboxService) + Handler (RegisterRoutes mounts the PUBLIC HMAC webhook POST /webhooks/mailgun/inbound only when svc+signingKey configured, + always the authed GET /inbox-address) + inbound_email.go (provider-neutral InboundEmail/InboundAttachment + verifyMailgunSignature) + the in-package TestInlineAttachmentNames. Imports internal/attachments (CaptureFromReceipt/FindDuplicateReceipt) + internal/htmlrender. db/email_inbox aliased `emailinboxdb` inside; in main it's `dbemailinbox`, internal/emailinbox is `emailinbox`. NewHandler(svc, signingKey) takes a possibly-nil svc (then /inbox-address reports enabled:false). The harness injects an inline runner via Service.SetRunInBackground.
│   #   isUniqueViolation (SQLSTATE 23505, was a root helper in email_inbox_service.go) moved to internal/kernel (kernel.IsUniqueViolation, in pgerr.go) — shared with banking_test.
...
│   ├── seeds/           # Reproducible seed data (e.g. categories.sql — the shared Chart of Accounts)
...
Config/tooling at the root: `go.mod` / `go.sum` (deps), `.env` (local config — `DATABASE_URL`, `PASETO_SYMMETRIC_KEY`, `PORT`, `GCS_BUCKET` for receipt-attachment storage, `DOCAI_*` for Document AI OCR, `INBOX_DOMAIN` + `MAILGUN_INBOUND_SIGNING_KEY` for the email-to-expense channel, and `GOTENBERG_URL` for HTML-body rendering, and `ANTHROPIC_API_KEY` (+ optional `TALA_MODEL`) for the Tala AI assistant), `.gitignore`, and this `CLAUDE.md`.
...
- **Deliberately left out:** DB-type glue (`pgNullText`, `pgNumericFromDecimal`, …) and the proto-unpacking `moneyToMinor` (Google `MoneyValue`, in `internal/ocr/documentai.go`) stay with their callers — the kernel takes no DB or proto dependency.
...
There are two upload paths. **"Add file"** (`POST /expenses/:id/attachments`) attaches a receipt to an *existing* expense and runs **no** OCR. **"Smart Upload"** (`POST /expenses/capture`) is receipt-first: it creates a **skeleton draft** (`needs_review=TRUE`, placeholder `'251'` Sundry Expenses category resolved from the shared CoA via `categories.GetCategoryByNominal` — `attachments.PlaceholderCategoryNominal`, `gross=0`), attaches the file, and kicks off **background OCR** (`OcrService.Enqueue` → a goroutine). The user picks Receipt or Invoice (`document_type`), which routes to the matching Document AI processor (Expense vs Invoice parser); residency is enforced by the **`eu` regional endpoint**. OCR drives the attachment's `ocr_status` (PENDING→PROCESSING→COMPLETE/FAILED/SKIPPED) and **COALESCE-fills only empty expense fields** — it never overwrites user-entered data and never clears `needs_review` (a human confirms by saving, which clears it via the normal update). `needs_review` is a **third axis**, orthogonal to the approval `status` and the attachment `ocr_status`. The Document AI call sits behind the `DocumentExtractor` interface (`internal/ocr`), so tests fake it (like the only-mock-external-services rule) while still using real Postgres + GCS; money is converted `MoneyValue`→pence with `shopspring/decimal` (HALF_UP). OCR is optional: with `DOCAI_*` unset, Smart Upload still creates drafts but they stay PENDING. An **opt-in** integration test (`TestDocumentAILive`, gated on `DOCAI_LIVE_TEST=1` so routine runs aren't billed) exercises the *real* API against both processors: `DOCAI_LIVE_TEST=1 go test -run TestDocumentAILive -v`.
...
- **Delete is soft + guarded by "in use".** `SoftDeleteContact` sets BOTH `deleted_at = now()` and `is_active = false` (the two lifecycle columns kept coherent; reads still filter on `deleted_at IS NULL` only). Before deleting, the service checks the contact isn't still referenced and returns **409 `conflict`** if it is — the FK can't enforce this because a soft-delete is an UPDATE (the row stays), so projects' `contact_id → contacts(id)` never fires. The check is the **`ContactHasProjects`** query in the **projects** sqlc domain (`db/queries/projects.sql`), consumed via a cross-domain `projects.Querier` injected into `contacts.NewService` (same pattern as the `auth.Querier`); extend its `EXISTS` with `OR EXISTS(...)` as more entities reference contacts (e.g. invoices). `GetContact` surfaces the same fact as a derived **`in_use`** bool on `ContactResponse` (computed only on GET) so the SPA's contact edit page hides the **Delete** button when the contact is in use. Tested in `contact_service_test.go` (`TestContactInUseFlag` + the delete-guard subtests).

### Invoices module

A **sales invoice** an organisation issues to one of its contacts (modelled on the FreeAgent Invoice resource), with embedded **line items**. A standalone domain in **`internal/invoices`** (`invoices.Service` + `Handler` self-registering `/api/v1/invoices` + DTOs + `status.go`), over its own sqlc package `db/invoices` (aliased `invoicesdb` inside; the cross-domain contacts querier is `contactsdb`). A deliberately **MINIMAL** first cut — the larger FreeAgent surface (ec_status, discount, CIS, second sales tax, per-line category/project, multi-currency, …) is deferred (see `BACKLOG.md`).

- **Money is BIGINT pence; VAT is ADDED ON TOP.** Header totals (`net/sales_tax/total/paid/due`) are `BIGINT` minor units — invoice totals can exceed the int32 ceiling. Each line stores `quantity` (NUMERIC), a VAT-exclusive `price_minor`, and `sales_tax_rate_bps`; the service computes `line_net = round(quantity × price_minor)` and `line_vat = money.AddOnVAT(line_net, rate_bps)` — **add-on VAT, the INVERSE of the expenses path's `money.ComputeFixedVAT`** (which EXTRACTS VAT already contained in a gross); don't swap them. The API carries money as **decimal pound strings** and VAT as **percentage strings** (`money.PercentToBps` / `money.BpsToPercentString`). `due_value_minor` is a generated DB column (total − paid).
- **Status = stored lifecycle + DERIVED display.** The stored `status` is only `DRAFT|SCHEDULED|SENT|WRITTEN_OFF|REFUNDED` (mirrors the CHECK); the user-facing `display_status` (Open/Overdue/Paid/Overpaid/Zero Value) is **derived at read time** from `due_on` + `total` + `paid` (`deriveDisplayStatus`), so it never goes stale. The lifecycle is data in `status.go` (`invoiceActions`) driven by **`POST /api/v1/invoices/:id/status`** with an `{"action": …}` discriminator (issue/schedule/send/write_off/refund/reopen): an illegal action from the current status is **409**, an unknown action is **400** (`oneof` binding).
- **Line items are embedded + rebuilt.** Create/update take the lines in the payload; update **replaces** them in one transaction (`DeleteInvoiceItemsForInvoice` + re-`CreateInvoiceItem` + `UpdateInvoiceTotals`). Each mutation (create/update/delete/status) loads + guards + writes inside one `kernel.WithTx` to close the check-then-write race.
- **Auth + editability.** Any active member may create/read/list; **update/delete/status require the invoice's creator or an owner/admin** (mirrors contacts). Only a **DRAFT** is editable/deletable (else **409**). `contact_id` is validated against the caller's org (`validateContact`, like projects); cross-tenant → **404**. Org-scoped + soft-deleted throughout. Tested in `invoice_service_test.go` + `money/money_test.go`.
- **Reference is REQUIRED + auto-numbered (org-level "global" sequence), SELF-HEALING.** **`GET /api/v1/invoices/next-reference`** suggests the next number zero-padded (`1 → "001"`) to pre-fill the create form. The suggestion is **`GREATEST(stored counter floor, highest numeric reference in use + 1)`** — i.e. it derives from the references actually in use (`MaxNumericInvoiceReference`, live invoices only, non-numeric refs ignored) clamped up to the stored `organisations.next_invoice_number` floor. That **max term is what makes it self-healing**: even if the stored counter drifts behind the real references (a manual/out-of-band reference, an import, …), the suggestion still lands one past the highest used number and never re-suggests a taken one. On create, a **numeric** reference raises the counter floor to `used + 1` via the `RaiseInvoiceNumber` auth query (a `GREATEST(...)` UPDATE — it only ever moves the floor **up**, so numbers are **never reused even after an invoice is deleted**, and it's concurrency-safe); a custom/non-numeric reference leaves the floor alone. `reference` is **required** (400 if missing, 422 if whitespace-only) and **unique per org** (`uq_invoices_reference`; a duplicate is **409**); the user may always override the suggestion. The invoices service reaches this via `auth.GetNextInvoiceNumber` + `invoices.MaxNumericInvoiceReference` (read) and `auth.New(tx).RaiseInvoiceNumber` (the in-transaction floor raise). Per-PROJECT sequences (`projects.project_invoice_sequence`) are deferred — see BACKLOG.
- **Payments land via the banking "Invoice Receipt".** `paid_value_minor` is no longer always 0: explaining a money-in bank transaction as an **`INVOICE_RECEIPT`** (the banking explain flow) sets `bank_transaction_explanations.paid_invoice_id` and re-syncs the invoice's `paid_value_minor = Σ(live INVOICE_RECEIPT explanations for it)` inside the explanation's transaction (so the derived display flips to **Paid** / stays Open while partially paid). Only a **SENT, not-fully-paid** invoice is payable, and a receipt may not exceed the invoice's outstanding `due_value_minor` (overpayment → **422**; split the bank line instead). Conversely, once a SENT invoice has **any** payment recorded, it **can't be reopened** to DRAFT (`ChangeStatus` → **409**) — reopening is the only route back to editing, and an edit could otherwise push the total below `paid`; the receipt(s) must be removed first. The banking service holds a cross-domain `*invoicesdb.Queries` (`SumInvoiceReceiptsForInvoice` + `UpdateInvoicePaidValue`) for this. **`GET /api/v1/invoices/outstanding`** (any active member) backs the receipt picker — the org's SENT invoices with `due_value_minor > 0`. A dedicated payments table + `paid_on`/`written_off_date` columns remain deferred (see BACKLOG). Tested in `banking_service_test.go` (`TestInvoiceReceiptExplain`) + `invoice_service_test.go` (`TestListOutstandingInvoices` + `TestReopenGuardWithPayments`).
- **Frontend (Vue SPA).** `web/src/views/Invoice{List,Entry,Detail}View.vue` + `components/InvoiceItemDialog.vue` (the "New invoice item" modal), over `services/invoices.service.ts` / `types/invoice.ts` / `lib/invoiceStatus.ts`. **Header-first flow** (FreeAgent-style): create makes an empty DRAFT → the detail document adds/edits line items via the modal. Because the PUT **rebuilds all lines**, every line add/edit/delete (and the header edit) re-sends the **full `items` array**. Status UI is the **Draft→Sent→Paid tracker** + **Mark as sent** (issue) / **Back to draft** (reopen); line-item editing + Edit/Delete are **DRAFT-only**. The form's **Payment terms (days)** is converted to `due_on` client-side. No client VAT math — the detail shows the API's per-line + header pound strings.
...
- **Module shape.** The `integrations` DB domain (`db/schema/integrations_schema.sql`, `db/queries/integrations.sql`, sqlc block → `db/integrations`) has three tables: `provider_credentials` (the **GLOBAL** per-provider OAuth app `client_id`/`client_secret`, shared by all orgs, **no `organisation_id`**), `organisation_integrations` (per-(org,provider) connection state — the access/refresh tokens + `connected_at`, **created by an UPSERT on first connect**) and `integration_expense_pushes` (the outcome/idempotency ledger). The Go code is split into `internal/integrations` (the shared, provider-agnostic machinery — `Service`, `Handler`, the workflow endpoints, the `OAuthClient` interface) and `internal/integrations/freeagent` (the FreeAgent client + `ProviderKey`), so a future Xero is a sibling provider package implementing `OAuthClient`. New env: `PUBSUB_EXPENSE_APPROVED_TOPIC`, `GOOGLE_CLOUD_PROJECT`, `WORKFLOW_SERVICE_ACCOUNT`, `API_PUBLIC_URL` (the BACKEND's own URL, for the OAuth redirect — distinct from the frontend `APP_BASE_URL`), `FREEAGENT_SANDBOX`. Provisioning + the data-residency note (infra runs in `europe-west1` = EU, not UK) are in `deploy-expensepush/README.md`. Tested in `integration_service_test.go`, `integration_internal_test.go`, `events_test.go` (real Postgres; FreeAgent + Pub/Sub faked; OIDC's positive path validated at deploy). Token encryption at rest + a transactional outbox are deferred — see `BACKLOG.md`.

### Tala (AI accountant assistant)

**Tala** is the in-app AI assistant — a UK-accountant chatbot that answers questions over the organisation's own data, explains how Kontala works, offers light advice/predictions, and can **propose changes the user confirms**. It runs an Anthropic **tool-use loop INSIDE the monolith** (no separate service, no Managed Agents), so its tools reuse the existing org-scoped services and the data never leaves our infra (the UK/EU residency requirement). The whole module is **`internal/tala`** — a leaf (nothing imports it), so it may depend on every other domain service (like `integrations` → `*expenses.Service`). New Go dependency: `github.com/anthropics/anthropic-sdk-go`. **No DB/sqlc domain** — it's pure Go over the other services.

- **Endpoint + shape.** `POST /api/v1/tala/chat` (behind `kernel.AuthMiddleware`; any active member — per-tool authorisation stays in each service). **Stateless**: the SPA holds the running history and re-sends it each turn (`ChatRequest.messages`); the response is `{reply, proposed_actions[], tool_calls[]}`. Non-streaming JSON. Files: `service.go` (the loop), `client.go`, `tools.go`, `propose.go`, `system.go` + `tala_system.md` (the embedded system prompt: persona + a "how Kontala works" guide + grounding + confirmation protocol + disclaimer), `handler.go`, `dto.go`.
- **The loop is hand-written, not the SDK tool runner** (`RunTurn`), so every tool call is org-scoped, audit-logged (`slog`), and iteration-capped (8), and the LLM sits behind a `modelClient` interface — the ONE thing tests fake (`service_test.go`; the tools run for real). Model `claude-opus-4-8` (override via `TALA_MODEL`), adaptive thinking, the system prompt prompt-cached.
- **Two non-negotiable invariants.** (1) **Org/user come from the token, never the model** — no tool input schema carries an org/user id; the executor injects `GetAuthOrgID`/`GetAuthUserID`. `TestNoToolExposesOrgOrUserInSchema` guards this across the whole tool set. (2) **The agent never mutates.** "Guarded writes" are **propose-only** tools (`propose_create_expense`, `propose_approve_expense`) that return a validated `ProposedAction`; the SPA renders a confirm card and, on **Confirm**, calls the EXISTING endpoint (`POST /expenses`, `POST /expenses/:id/status`). So a prompt injection or model mistake can, at worst, propose something the user then declines.
- **Read tools** are thin org-scoped wrappers over existing service reads across expenses/invoices/bills/banking/vat/reports/overview, each returning the service DTO as JSON (the tool never names the DTO type — `jsonResult(any)`).
- **Optional, gated on `ANTHROPIC_API_KEY`** (like GCS/DocAI/Pub/Sub): unset → the route isn't mounted and startup logs `tala: ... AI assistant disabled`. Built in `main.go` from the existing service instances.
- **Frontend (Vue SPA).** `web/src/views/TalaChatView.vue` — a simple prompt screen (transcript, example-prompt chips, "Tala is thinking…" indicator, composer, proposal cards) **wired as the FIRST item in the top nav** (`AppTopBar.vue` `navItems`, route `/tala`). Over `services/tala.service.ts` + `types/tala.ts`; the proposal card (`components/TalaProposalCard.vue`) calls the existing expense endpoints on Confirm. Deferred work (SSE streaming, persisted threads, in-loop writes, more domains, cost controls) is in `BACKLOG.md`.
...
- **Where new code goes (modular-monolith migration in progress).** The codebase is moving off the flat root `package main` into a modular monolith:
  - **Shared kernel → `internal/kernel`.** The cross-cutting infrastructure every domain uses — `AppError`/error codes, the HTTP helpers (`RespondError`, `BindJSON`, `AuthMiddleware`, `GetAuthUserID`/`OrgID`), `AuthorizeMember`/`IsOrgAdmin`, the shared pgtype conversions (`NullText`, `Int32FromPtr`, …), and `WithTx`. Call these as `kernel.X`. The kernel must stay foundational — it may not import any domain package (enforced by `TestKernelHasNoDomainImports`).
  - **Domain/feature code → `internal/<domain>`** (e.g. `internal/integrations/freeagent`), NOT the repo root.
  - **The repo root `package main` is wiring-only** — the migration is **COMPLETE**. The only non-test root files are `main.go` (dependency wiring + startup) and `server.go` (the thin HTTP shell: engine + CORS + `/health` + static SPA + `Router()`); everything else lives in `internal/<domain>` or `internal/kernel`, referenced directly as `kernel.*` (the old transitional `aliases.go` bridge has been deleted). `arch_test.go`'s `TestRootPackageIsWiringOnly` (runs under `go test ./...`) **fails the build if a new `.go` file appears in the root** — so put new code in `internal/<domain>` or `internal/kernel`, never the root.
