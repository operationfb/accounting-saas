...
- **Reference is REQUIRED + auto-numbered (org-level "global" sequence).** Each org has a `next_invoice_number` counter (a column on `organisations`); **`GET /api/v1/invoices/next-reference`** returns it zero-padded (`1 → "001"`) to pre-fill the create form. `reference` is **required** (400 if missing, 422 if whitespace-only) and **unique per org** (`uq_invoices_reference`; a duplicate is **409**). Creating an invoice whose reference IS the suggested number **advances the counter** (the `BumpInvoiceNumber` auth query, guarded `WHERE next_invoice_number = $used` so a custom / out-of-sequence reference doesn't move it); the user may always override the reference. The invoices service reaches the counter via the auth queries `GetNextInvoiceNumber` / `BumpInvoiceNumber` (+ `auth.New(tx)` for the in-transaction bump). Per-PROJECT sequences (`projects.project_invoice_sequence`) are deferred — see BACKLOG.
