...
- **OAuth is the monolith's job** (interactive, one-time per org). The app's OAuth **`client_id`/`client_secret` are GLOBAL** — one row per provider in the `provider_credentials` table, shared by every org (in OAuth they identify OUR application, not a tenant) and **managed directly in the DB** (the app only READS them — there is deliberately no save-credentials endpoint). `freeagent_client.go` is an **auth-only** FreeAgent client (`ExchangeCode`/`RefreshToken`/`authorizeURL`); `integration_service.go` (`IntegrationService`) runs the connect flow — build the authorize URL from the global `client_id` (signed-`state` CSRF via the existing PASETO `tokenMaker`), handle the public callback, store/refresh the **per-org** tokens, status, disconnect. The provider key (`"freeagent"`) is `IntegrationService` **instance config**, not a hardcoded constant, so tests use a throwaway key (isolated from the real row on the shared dev DB) and a future Xero is just another instance. Handlers in `integration_handler.go`, routes in `server.go` under `/api/v1/integrations/freeagent` (status + disconnect, owner/admin) and `/api/v1/freeagent/{connect,callback}` (connect returns the authorize URL as **JSON** so the bearer-token SPA navigates itself; callback is **public** and 302s back to the SPA). Access tokens last ~1h (refreshed server-side ~5 min early); refresh tokens ~20y.
