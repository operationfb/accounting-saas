...
//
// NOTE: this package is itself named `storage`, so the Google SDK (also package
// `storage`) is imported under the alias `gcs`.
// =============================================================================
...
// NewGCS builds a gcsStorage for the named bucket using ADC. It returns the
// Storage interface so callers depend only on the abstraction.
func NewGCS(ctx context.Context, bucket string) (Storage, error) {
